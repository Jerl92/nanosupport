...
// A callback function to add a custom field to our "presenters" taxonomy  
function presenters_taxonomy_form_factor_fields($tag) { 
    // Check for existing taxonomy meta for the term you're editing  
     $t_id = $tag->term_id; // Get the ID of the term you're editing
     $get_term_color = get_term_meta($t_id, 'meta_color', true);
 ?>  
 ...
 // A callback function to save our extra taxonomy field(s)  
 function save_taxonomy_form_factor_meta( $term_id ) {
    if ( isset($_POST['meta_color']) ) {
        update_term_meta( $term_id, 'meta_color', esc_attr($_POST['meta_color']) );
    }
} 
...
// Add the fields to the "presenters" taxonomy, using our callback function  
add_action( 'nanosupport_form_factor_edit_form_fields', 'presenters_taxonomy_form_factor_fields', 10, 2 );  
add_action( 'nanosupport_form_factor_add_form_fields', 'presenters_taxonomy_form_factor_fields', 10, 2 ); 

// Save the changes made on the "presenters" taxonomy, using our callback function  
add_action( 'edited_nanosupport_form_factor', 'save_taxonomy_form_factor_meta', 10, 2 ); 
add_action( 'create_nanosupport_form_factor', 'save_taxonomy_form_factor_meta', 10, 2 ); 
...
function ns_set_custom_columns( $columns ) {
    ...
    $new_columns = array(
            ...
            'modified_date'     => __( 'Modified date', 'nanosupport' ),
...
function my_column_register_sortable( $columns ) {
    ...
    $columns['ticket_responses'] = 'ticket_responses';
    return $columns;
}

add_filter("manage_edit-nanosupport_sortable_columns", "my_column_register_sortable" );
...
function ns_populate_custom_columns( $column, $post_id ) {
    ...
    switch ( $column ) {
        ...
        case 'modified_date' :
            $post = get_post( $post_id );
            echo date( 'd M Y h:iA', strtotime( ns_get_ticket_modified_date($post->ID) ) );
            break;
